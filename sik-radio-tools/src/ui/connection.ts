  ...
  container.innerHTML = `
    <div class="connection-bar">
      <div class="connection-status">
        ...
        ${(state.transport as { portInfo?: { name?: string } })?.portInfo?.name ? `<span class="text-secondary">(${(state.transport as { portInfo?: { name?: string } }).portInfo!.name})</span>` : ''}
      </div>
...
function getStatusText(s: ConnectionState): string {
  switch (s) {
...
async function handleDisconnect(state: AppState, setState: (p: Partial<AppState>) => void): Promise<void> {
  if (state.transport) {
    try {
      await (state.transport as { close(): Promise<void> }).close();
    } catch {
      /* ignore */
    }
  }
